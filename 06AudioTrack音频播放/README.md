...
JNIEXPORT void JNICALL Java_com_dongnaoedu_dnffmpegplayer_JasonPlayer_sound
  (JNIEnv *env, jobject jthiz, jstring input_jstr, jstring output_jstr){
	...
	//AudioTrack对象   (II)Landroid/media/AudioTrack：为方法签名，可以通过 javap -s 命令获取对应方法的签名
	jmethodID create_audio_track_mid = (*env)->GetMethodID(env,player_class,"createAudioTrack","(II)Landroid/media/AudioTrack;");//得到AudioTrack对象
	jobject audio_track = (*env)->CallObjectMethod(env,jthiz,create_audio_track_mid,out_sample_rate,out_channel_nb);

	//调用AudioTrack.play方法
	jclass audio_track_class = (*env)->GetObjectClass(env,audio_track);
	jmethodID audio_track_play_mid = (*env)->GetMethodID(env,audio_track_class,"play","()V");//拿到play方法
