...
JNIEXPORT void JNICALL Java_com_dongnaoedu_dnffmpegplayer_VideoUtils_decode
  (JNIEnv *env, jclass jcls, jstring input_jstr, jstring output_jstr){
	...
	struct SwsContext *sws_ctx = sws_getContext(
			...
			SWS_BILINEAR, NULL, NULL, NULL);//像素格式转换，也可以进行缩放
	...
	while(av_read_frame(pFormatCtx,packet) >= 0){
		...
		if(got_frame){
			...
			sws_scale(sws_ctx,
					...
					yuvFrame->data, yuvFrame->linesize);//像素格式转换，也可以进行缩放

			//向YUV文件保存解码之后的帧数据
			//AVFrame->YUV
			//一个像素包含一个Y
			int y_size = pCodeCtx->width * pCodeCtx->height;//总共有多少Y
			fwrite(yuvFrame->data[0], 1, y_size, fp_yuv);//Y
			fwrite(yuvFrame->data[1], 1, y_size/4, fp_yuv);//U等于Y的1/4
			fwrite(yuvFrame->data[2], 1, y_size/4, fp_yuv);//V等于Y的1/4
